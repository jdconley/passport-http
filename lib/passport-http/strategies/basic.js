 ...
 *   - `allowEmptyUserId`  a userid can be empty in the authorization data, defaults to false
 *   - `allowEmptyPassword`  a password can be empty in the authorization data, defaults to false
 *
 * Examples:
 *
...
function BasicStrategy(options, verify) {
  ...
  this._allowEmptyUserId = options.allowEmptyUserId ? true : false;
  this._allowEmptyPassword = options.allowEmptyPassword ? true : false;
}
...
BasicStrategy.prototype.authenticate = function(req) {
  ...
  // Password can contain a colon, so have to account for that. Can't just split.
  var decodedAuth = new Buffer(parts[1], 'base64').toString();
  var separatorIndex = decodedAuth.indexOf(":");
  
  if (separatorIndex < 0) { return this.fail(400); }
  
  var userid = decodedAuth.substr(0, separatorIndex);
  var password = decodedAuth.substr(separatorIndex + 1);
  
  // userid and password are not required by the spec.
  // This behavior is retained as an option (default to false) for compatibility 
  // with prior versions of this module.
  if ((!this._allowEmptyUserId && !userid) || (!this._allowEmptyPassword && !password)) {
